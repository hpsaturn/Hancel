...
public class ContactoDAO extends SQLiteHelper{
	...
	public Cursor getList(){
		return super.mDb.rawQuery("select * from "+ DBConstants.TABLE_CONTACTS,
				null);
	}
	
	public int getList(int idPadre){
		int id=0;
		Cursor c=super.mDb.rawQuery("select * from " + DBConstants.TABLE_CONTACTS
				+ " where " + DBConstants.IdPadre + " = " + idPadre, null);		
		if(c.moveToFirst()){
			id = c.getInt(0);
		}		
		return id;
	...
	public boolean deleteTurno(int mPadre) {				
		return super.mDb.delete(DBConstants.TABLE_CONTACTS, DBConstants.IdPadre
				+ " = " + mPadre, null) > 0;
	}
	
	public boolean updateTurno(int id,int mPadre,String mNom) {
		ContentValues newValues = new ContentValues();		
		newValues.put(DBConstants.IdPadre, mPadre);
		newValues.put(DBConstants.telefonos, mNom);		
		return super.mDb.update(DBConstants.TABLE_CONTACTS, newValues, 
	...
	public long Insertar(int mPadre,String mNom,String emailList) {		
		...
		newValues.put(DBConstants.IdPadre, mPadre);
		newValues.put(DBConstants.telefonos, mNom);
		newValues.put(DBConstants.emails, emailList);
		System.out.println("Se quiere insertar en la tabla de Equipos ---> " + newValues.toString());		
		return super.mDb.insert(DBConstants.TABLE_CONTACTS, null, newValues);
	}		
}